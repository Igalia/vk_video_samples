...
bool VulkanAV1Decoder::BeginPicture(VkParserPictureData* pnvpd)
{
    ...
    for (int i = 0; i < 7; i++) {
        ...
        av1->active_ref_names[i] = active_ref_names[i];
...
int VulkanAV1Decoder::SetupFrameSizeWithRefs()
{
    ...
    for (i = 0; i < REFS_PER_FRAME; ++i) {
        ...
        if (tmp) {
            found = 1;
            VkPicIf *m_pPic = m_pBuffers[active_ref_names[i]].buffer;
            if (m_pPic) {
...
void VulkanAV1Decoder::DecodeSegmentationData()
{
    ...
    if (pic_info->segmentation_update_data) {
        ...
        if (primary_ref_frame != PRIMARY_REF_NONE) {
            ...
            int prim_buf_idx = active_ref_names[primary_ref_frame];
...
void VulkanAV1Decoder::DecodeLoopRestorationData()
{
    ...
    if (use_lr)  {
        ...
        pic_info->lr_unit_shift = lr_unit_shift;
    } else {
        for (int pl = 0; pl < n_planes; pl++)
            pic_info->lr_unit_size[pl] = 3;  // 256
    }
    pic_info->lr_uv_shift = 0;
    if (!sps->color_config.flags.mono_chrome) {
        if (use_chroma_lr && (sps->color_config.subsampling_x && sps->color_config.subsampling_y)) {
            pic_info->lr_uv_shift = u(1);
            pic_info->lr_unit_size[1] = pic_info->lr_unit_size[0] - pic_info->lr_uv_shift; // *std::min(sps->subsampling_x, sps->subsampling_y));
...
void VulkanAV1Decoder::SetFrameRefs(int last_frame_idx, int gold_frame_idx)
{
    ...
    for (int i = 0; i < NUM_REF_FRAMES; i++) {
        int ref_frame_name = ref_frame_names[i];
        Ref_OrderHint = RefOrderHint[ref_frame_name];
...
int VulkanAV1Decoder::IsSkipModeAllowed()
{
    ...
    for (int i = 0; i < REFS_PER_FRAME; i++) {
        int frame_idx = active_ref_names[i];
        if (frame_idx != -1) {
...
bool VulkanAV1Decoder::ParseObuFrameHeader()
{
    ...
    if (sps->flags.reduced_still_picture_header) {
        ...
        if (show_existing_frame) {
            ...
            int32_t show_existing_frame_index = ref_frame_names[frame_to_show_map_idx];
    ...
    if (IsFrameIntra()) {
        ...
        // if (pbi->need_resync != 1)
        {
            ...
            if (frame_refs_short_signaling) {
                ...
                const int lst_idx = ref_frame_names[lst_ref];

                const int gld_ref = u(REF_FRAMES_BITS);
                const int gld_idx = ref_frame_names[gld_ref];
            ...
            for (int i = 0; i < REFS_PER_FRAME; i++) {
                if (!frame_refs_short_signaling) {
                    ...
                    int ref_frame_name = ref_frame_names[ref_frame_index];

                    if (ref_frame_name == -1) {
                        assert(!"invalid reference");
                    }
                    active_ref_names[i] = ref_frame_name;
                } else {
                    int ref_frame_name = ref_frame_names[ref_frame_idx[i]];
                    active_ref_names[i] = ref_frame_name;
                }
                ...
                if (sps->flags.frame_id_numbers_present_flag) {
                    ...
                    if (ref_id != ref_frame_id[active_ref_names[i]] || RefValid[active_ref_names[i]] == 0) {
                        assert(!"Ref frame ID mismatch");
                    }
                }
            }
