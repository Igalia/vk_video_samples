...
bool VulkanVP9Decoder::ParseByteStream(const VkParserBitstreamPacket* pck, size_t* pParsedBytes)
{
    ...
    while ((dataSize > 0) || m_pictureStarted) {
        if (!m_pictureStarted) {
            ...
            if ((dataSize > m_bitstreamDataLen) && !resizeBitstreamBuffer(dataSize - m_bitstreamDataLen)) {
                return false;
            }
...
bool VulkanVP9Decoder::ParseFrameHeader(const uint8_t* pBitstream, uint32_t framesize)
{
